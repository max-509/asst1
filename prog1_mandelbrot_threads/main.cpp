...
extern void mandelbrotThreadEffective(
        int numThreads,
        float x0, float y0, float x1, float y1,
        int width, int height,
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, "t:v:e:?", long_options, NULL)) != EOF) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (isEfficient) {
            double startTime = CycleTimer::currentSeconds();
            mandelbrotThreadEffective(numThreads, x0, y0, x1, y1, width, height, maxIterations, output_thread);
            double endTime = CycleTimer::currentSeconds();
